...
# Directory Paths
directory_path <- "./data/stepping_sampling/inverse_effect"
maf_output_dir <- file.path(directory_path, "inverse_maf_qq_plots")
wo_output_dir <- file.path(directory_path, "inverse_wo_qq_plots")
...
generate_effect_sizes <- function(n_loci, allele_frequencies) {
  
  # Inverse relationship: Higher allele frequencies lead to smaller effect sizes
  # We use 1 - allele_frequencies to invert the relationship
  effect_sizes <- (1 - allele_frequencies) * rnorm(n_loci)
  ...
  for (file_name in names(lists)) {
    ...
    for (replicate in names(json_data)) {
      ...
      for (loci_size in names(loci_data)) {
        ...
        allele_frequencies <- c(as.numeric(loci_data[[loci_size]]$allele_freq))
        
        num_loci <- as.numeric(sub("_loci", "", loci_size))  # Extract number from "1_loci", "10_loci", etc.
        effect_sizes <- generate_effect_sizes(num_loci, allele_frequencies)
...
# Load Data and Process MAF and WO
